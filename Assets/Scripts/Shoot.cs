...
{
    [SerializeField]
    float pullSpeed; // tworzymy sobie zmienna do operowania szybkosci naciagania
    ...
    [SerializeField]
    int numberOfArrows = 10; // ilosc strzal ktora bedzie operowal gracz

    [SerializeField]
    GameObject bow; // do tego bedziemy przypisywac prefab naszego luku

    float pullAmount = 0; // wartosc naciagu
    ...
    void ShootLogic() // funkcja odpowiadajaca za strzelanie
    {
        if (numberOfArrows > 0) // sprawdzamy czy jest wiecej strzal niz 0
        {
            ...
            //ProjectalAddForce _arrowProjectile = GetComponent<ProjectileAddForce>();
